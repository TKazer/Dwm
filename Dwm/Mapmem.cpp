...
BOOL util::MapMem::GetMapMemFunction(PMemory_func func) {
	...
	DWORD64 Base = (DWORD64)GetModuleHandleA(XOR("kernelbase.dll"));
	...
	func->pOpenFileMappingW = (PVOID)((DWORD64)base + (DWORD64)GetProcAddress((HMODULE)Base, XOR("OpenFileMappingW")) - Base);
	func->pMapViewOfFileExNuma = (PVOID)((DWORD64)base + (DWORD64)GetProcAddress((HMODULE)Base, XOR("MapViewOfFileExNuma")) - Base);
	return TRUE;
}
